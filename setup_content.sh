#!/usr/bin/env bash
# This script runs setup for CONTENT in an OpenShift or Kubernetes environment.
...
TIMESTAMP=$(date +"%F-%T")
LOG_DIR=$(pwd)
LOG_FILE="${LOG_DIR}/setup_content-${TIMESTAMP}.log"
TEMP_LOG_FILE='setup_content.log'
...
fatal_ansible_not_installed() {
    log_error <<-EOF
		...
		You must install Ansible before you can install content.
...
# Change to the running directory for conf file and inventory file
# defaults.
